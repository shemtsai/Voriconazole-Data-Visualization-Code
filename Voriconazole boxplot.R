...
ggplot(data, aes(x = mg_kg_dose_bin, y = VoriLvlValue)) + 
  ...
  geom_boxplot(aes(fill = factor(mg_kg_dose_bin)), size = 1.25) +
  geom_jitter(aes(color = Within_Goal), width = 0.1, height = 0.1, size = 2) +
  # Add p-values
  ...
  annotate("segment", x = 1, xend = 2, y = max(data$VoriLvlValue, na.rm = TRUE) + 1, 
           yend = max(data$VoriLvlValue, na.rm = TRUE) + 1, size = 1.5) +  # Bracket between ≤2 and 2-3
  annotate("segment", x = 2, xend = 3, y = max(data$VoriLvlValue, na.rm = TRUE) + 1, 
           yend = max(data$VoriLvlValue, na.rm = TRUE) + 1, size = 1.5) +  # Bracket between 2-3 and ≥3
  # Add vertical lines at the bracket points
  ...
  scale_color_manual(values = c("Within Goal" = "black", "Outside Goal" = "red")) +
